...
{
    var SingleLineInput = Input.Aggregate((x, y) => x + y);

    var disabledRanges = GetDisabledRanges(DontRegex().Matches(SingleLineInput).Select(x => x.Index).ToList(),
                                    DoRegex().Matches(SingleLineInput).Select(x => (int?)x.Index).ToList());

    var matches = Day3.MulRegex().Matches(SingleLineInput);

    var inDoRange = matches.Where(match =>
    {
        return !disabledRanges.Any(range => range.start < match.Index && range.end > match.Index);
    }).ToList();

    return SumMatches(inDoRange);
}
private static List<(int start, int end)> GetDisabledRanges(List<int> dontIndices, List<int?> doIndices)
{
    List<(int start, int end)> disabledRanges = [(dontIndices.First(), doIndices.FirstOrDefault(x => x > dontIndices.First()) ?? int.MaxValue)];

    foreach (var dont in dontIndices.Skip(1))
    {
        if (dont > disabledRanges[^1].end)
            disabledRanges.Add((dont, doIndices.FirstOrDefault(x => x > dont) ?? int.MaxValue));
    }
...
internal static partial Regex DontRegex();
[GeneratedRegex(@"do\(\)")]
