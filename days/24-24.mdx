...
As is so often the case with Advent of Code, the first part is mostly just setting things up.

<br />
All we need to do is model the system of wires, run the system until it's in its
...
The puzzle gives us the critical piece of information without needing to deduce it: the system is simulating the addition of two binary numbers.

<br />

Once we know that, we can simplify the system a little to look for where logic gates
aren't currently correct for the <a href="https://www.electronics-tutorials.ws/combination/comb_7.html">binary addition</a> process, which is exactly what we're looking for. So I simulate some <a href="https://en.wikipedia.org/wiki/Adder_(electronics)">adders</a> to find the logic gate sets that aren't correct. It requires a little domain specific knowledge, but the actual code is pretty simple.
