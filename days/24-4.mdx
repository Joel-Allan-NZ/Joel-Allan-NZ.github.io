...
public override object PartOne()
{
  ...
  var count = Input.Sum(line => XmasRegex().Matches(line).Count + SamxRegex().Matches(line).Count);
  count += rotated.Sum(line => XmasRegex().Matches(line).Count + SamxRegex().Matches(line).Count);

  count += LeftToRightDownDiagonal(Input);
  count += LeftToRightDownDiagonal(rotated);
...
[GeneratedRegex(@"SAMX")]
internal static partial Regex SamxRegex();
```
...
- The 'M' and 'S' must be on opposite sides of the central 'A'. If we have two 'M's and two 'S's, then as long as one set of 'M' and 'S' are opposite, the other must also be.
  {' '}
  <br />
  From there it's simply a matter of checking 'A' values in valid locations, and
  confirming they have the correct corners around them.
  <br />
